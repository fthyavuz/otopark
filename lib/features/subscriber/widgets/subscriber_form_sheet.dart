...
class _SubscriberFormSheetState extends ConsumerState<SubscriberFormSheet> {
  ...
  void initState() {
    ...
    _feeCtrl = TextEditingController(
      text: ex?.subscriber.monthlyFee.toStringAsFixed(0) ?? '',
    );

    final now = DateTime.now();
    _startDate = ex?.subscriber.startDate ??
        DateTime(now.year, now.month, 1); // first of this month
    _endDate = ex?.subscriber.endDate ??
        DateTime(now.year, now.month + 1, 0); // last of this month
    ...
    if (ex != null && ex.plates.isNotEmpty) {
      ...
      _plateCtrls.add(TextEditingController()); // one empty row
    }

    // If no existing subscriber, pre-fill fee from active tariff
    if (ex == null) {
  ...
  void _addPlateRow() => setState(() => _plateCtrls.add(TextEditingController()));
    ...
    try {
      if (widget.existing != null) {
        ...
        await db.updateSubscriber(SubscribersCompanion(
          ...
          monthlyFee: Value(double.parse(_feeCtrl.text.trim())),
          isActive: Value(_endDate.isAfter(DateTime.now())),
        ));
  ...
  Future<bool> _showForeignPlateDialog(String plate) async {
    return await showDialog<bool>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: Text('"$plate" Türk plaka formatına uymuyor. Yine de eklensin mi?'),
            actions: [
              TextButton(
